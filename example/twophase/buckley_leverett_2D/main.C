...
int main(int argc, char **argv)
{
  ...
  while (sys.time < t_end)
  {
    ...
    if (t_step % 100 == 0)
    {
      snprintf(filename, sizeof(filename), "./output/atwo_bl_solution_2D_%d_%.0f.m", t_step,dt);
      PetscCall(SaveVecToMatlab(x, filename, "p_s"));
    }
    // if (t_step == 1500)
    // {
    //   snprintf(filename, sizeof(filename), "./output/two_bl_solution_2D_%d_%.0f.m", t_step,dt);
    //   PetscCall(SaveVecToMatlab(x, filename, "p_s"));
    // }
    // if (t_step == 2000)
    // {
...
PetscErrorCode FormFunction(SNES snes, Vec x, Vec f, void *ctx)
{
  ...
  for (const Polyhedron &e : mesh.elem_local_range())
  {
   ...
   if (s<0.0)
    ...
    {
      // PetscPrintf(PETSC_COMM_WORLD,"e(%d)--face.pos()=%d\n",p_dof,face.pos());
