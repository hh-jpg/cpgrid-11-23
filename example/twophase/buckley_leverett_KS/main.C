...
PetscErrorCode MyComputeRHS(DM dm, Vec b, void *ctx);
...
int main(int argc, char **argv)
{
  ...
  while (sysp.time < t_end)
  {
    ...
    sysp.time += dt;
    // 保存旧的解并进行求解
    PetscPrintf(PETSC_COMM_WORLD, "----------------- ksp_p -----------------\n");
    ...
    PetscCall(KSPSetOperators(ksp_p, jp, jpre)); // 使用 J 和 Jpre 作为 kspsolve 的操作矩阵
    // 获取右端项 b
    PetscCall(VecDuplicate(xp, &b));
    PetscCall(MyComputeRHS(dm_p, b, &syss));
    // SaveVecToMatlab(b, "./Test_output/b.m", "b");
    PetscCall(KSPSolve(ksp_p, b, xp));
    // std::cout<<"----+++__++-+++++"<<std::endl;
    PetscCall(KSPGetSolution(ksp_p, &xp));// 获取并保存解, x---p^{n+1} 、xold---S_w^{n}
    SaveVecToMatlab(xp, "./Test_output/p.m", "p");
    if (t_step % 10 == 0 )
    {
  ...
  SaveVecToMatlab(xs, "./Test_output/s.m", "s");
    if (t_step % 10 == 0 )
    {
...
PetscErrorCode MyComputeRHS(DM dm, Vec b, void *ctx) 
{
  PetscErrorCode ierr;
  PetscInt n;
  System *sysp,*syss = (System*) ctx;
  PetscScalar  value = 0.0, *s_old_array;
  // Vec &b=rhs;

  PetscCall(DMCPGetSystem_CP(dm, sysp));
  // 获取系统的DofMap和Mesh
  ...
  // VecDuplicate(b, &s_old_array);
  PetscCall(VecGetArray(syss->get_old_solution(), &s_old_array));
  ...
  for (const Polyhedron &e : mesh.elem_local_range())
  {
    ...
    for (const Face &face : e.get_faces())
    {
      if (face.neighbor() == nullptr)
      {
        if (face.pos() == 0)
        {
          value = f_in * face.compute_face_area();
        ...
        else if (face.pos() == 1)
        {
          ...
          value = face.compute_efftrans(mob_oc + mob_wc) * (p_out);
        }
      }
      else
      {
       ...
       PetscCall(VecSetValue(b, p_dof, value, INSERT_VALUES));
       
    // PetscPrintf(PETSC_COMM_WORLD,"pppp_%d=%.5e\n",p_dof,(p));
  }
    ierr = VecAssemblyBegin(b);CHKERRQ(ierr);
    ierr = VecAssemblyEnd(b);CHKERRQ(ierr);
    PetscCall(VecRestoreArray(sysp->get_old_solution(), &s_old_array));  // 恢复
